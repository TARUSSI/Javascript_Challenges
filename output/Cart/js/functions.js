...
const setButtonToInCart = (element) => {
  ...
  const imgTag = createImgTag("images/check.svg");
...
const addImageToCart = (foodItem, cart) => {
  const divTag = createDivTag();
  const imgSrc = `images/${menuItems[foodItem].image}`;
  const altName = menuItems[foodItem].alt;
...
const addNameToCart = (foodItem, cart) => {
  const divTag = createDivTag("content");
...
const addButtonsToCart = (cart) => {
  const divTag = createDivTag("quantity__wrapper");
  const decreaseButton = createButtonTag("decrease");
  const increaseButton = createButtonTag("increase");
...
const addPriceToCart = (foodItem, cart) => {
  const divTag = createDivTag("subtotal", `$${menuItems[foodItem].price}`);
  cart.append(divTag);
};
...
const addToCart = (element, list, cart) => {
  const indexOfElement = getIndex(element, list);
...
const costInCart = (cart) => {
  const individualCost = cart.querySelectorAll(".subtotal");
  let sumOfCost = 0;
...
const changeAmount = (foodItem, change) => {
  const amount = foodItem.querySelectorAll(".quantity");
...
const updateSubtotal = (foodItem) => {
  const cost = foodItem.querySelector(".price");
  const subtotal = foodItem.querySelector(".subtotal");
  const amount = foodItem.querySelectorAll(".quantity")[0].innerText;
  const total = (amount * getCost(cost)).toFixed(2);
...
const updateTotalCost = (topCart, bottomCart) => {
  const allCost = bottomCart.querySelectorAll(".amount");
  const label = bottomCart.querySelectorAll(".label");
  const subtotal = parseFloat(costInCart(topCart));
  const tax = (taxRate * subtotal).toFixed(2);
...
const findOnLeftCart = (foodItem, cart) => {
  const foodName = foodItem.querySelector(".menu-item").innerText;
  const indexOnLeft = menuItems.findIndex((x) => x.name === foodName);
  const item = cart.querySelectorAll("li");
...
const incDecAmount = (button, change) => {
  const foodItem = rightTopCart.querySelectorAll("li")[button];

  changeAmount(foodItem, change);

  if (isQuantityZero(foodItem)) {
    removeFromCart(foodItem);
    const itemOnLeft = findOnLeftCart(foodItem, leftCart);
